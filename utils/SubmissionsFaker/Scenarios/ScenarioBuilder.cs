...
public class ScenarioBuilder
{
    ...
    {
        PlatformAdmin = clientFactory.NewForAuthenticatedUser(SeederVars.PlatformAdminUsername,
            SeederVars.PlatformAdminPassword);
    ...
    {
        var createdNgo =
            PlatformAdmin.PostWithResponse<ResponseWithId>("/api/ngos", new { name = $"{ngo}-{Guid.NewGuid()}"});
        ...
        Action<ElectionRoundScenarioBuilder>? cfg = null)
    {
        ...
        var createdElectionRound = PlatformAdmin.PostWithResponse<ResponseWithId>("/api/election-rounds",
            new
            {
                countryId = "6984f722-6963-d067-d4d4-9fd3ef2edbf6",
                title = title,
                englishTitle = title,
