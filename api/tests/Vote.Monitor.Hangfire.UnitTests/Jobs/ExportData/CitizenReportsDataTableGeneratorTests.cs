...
public class CitizenReportsDataTableGeneratorTests
{
    ...
    [
        SelectOption.Create(_option1Id, _option1Text),
        SelectOption.Create(_option2Id, _option2Text),
        SelectOption.Create(_option3Id, _option3Text, false, true),
        SelectOption.Create(_option4Id, _option4Text, true)
    ];


    private static readonly Guid _textQuestionId = Guid.NewGuid();

    private static readonly TextQuestion _textQuestion =
        TextQuestion.Create(_textQuestionId, TextQuestionCode, _questionText);

    private static readonly Guid _numberQuestionId = Guid.NewGuid();

    private static readonly NumberQuestion _numberQuestion =
        NumberQuestion.Create(_numberQuestionId, NumberQuestionCode, _questionText);

    private static readonly Guid _dateQuestionId = Guid.NewGuid();

    private static readonly DateQuestion _dateQuestion =
        DateQuestion.Create(_dateQuestionId, DateQuestionCode, _questionText);

    private static readonly Guid _ratingQuestionId = Guid.NewGuid();

    private static readonly RatingQuestion _ratingQuestion =
        RatingQuestion.Create(_ratingQuestionId, RatingQuestionCode, _questionText, RatingScale.OneTo10);

    private static readonly Guid _singleSelectQuestionId = Guid.NewGuid();

    private static readonly SingleSelectQuestion _singleSelectQuestion =
        SingleSelectQuestion.Create(_singleSelectQuestionId, SingleSelectQuestionCode, _questionText, _selectOptions);

    private static readonly Guid _multiSelectQuestionId = Guid.NewGuid();

    private static readonly MultiSelectQuestion _multiSelectQuestion =
        MultiSelectQuestion.Create(_multiSelectQuestionId, MultiSelectQuestionCode, _questionText, _selectOptions);
    ...
    [Fact]
    public void CitizenReportsDataTableGenerator_Should_Generate_DataTable_With_Default_Columns()
    {
        // Arrange
        var generator = CitizenReportsDataTable
    ...
    [Fact]
    public void CitizenReportsDataTableGenerator_Should_Generates_Correct_DataTable_WhenTextAnswers()
    {
        // Arrange
        var form = Fake.Form(DefaultLanguageCode, _textQuestion);

        // No notes, no attachments
        var citizenReport1 = Fake.CitizenReport(form.Id, TextAnswer.Create(_textQuestionId, "answer 1"), [], []);
        // No notes, 2 attachments
        var citizenReport2 = Fake.CitizenReport(form.Id, TextAnswer.Create(_textQuestionId, "answer 2"), [],
            FakeAttachmentsFor(_textQuestionId));
        // 2 notes, no attachments
        var citizenReport3 = Fake.CitizenReport(form.Id, TextAnswer.Create(_textQuestionId, "answer 3"),
            FakeNotesFor(_textQuestionId), []);
        // 2 notes, 2 attachments
        var citizenReport4 = Fake.CitizenReport(form.Id, TextAnswer.Create(_textQuestionId, "answer 4"),
            FakeNotesFor(_textQuestionId), FakeAttachmentsFor(_textQuestionId));

        List<object[]> expectedData =
        [
            [.. GetDefaultExpectedColumns(citizenReport1), "answer 1", 0, "", "", 0, "", ""],
            [.. GetDefaultExpectedColumns(citizenReport2), "answer 2", 0, "", "", 2, Attachment1Url, Attachment2Url],
            [.. GetDefaultExpectedColumns(citizenReport3), "answer 3", 2, Note1, Note2, 0, "", ""],
            [
                .. GetDefaultExpectedColumns(citizenReport4), "answer 4", 2, Note1, Note2, 2, Attachment1Url,
                Attachment2Url
            ],
        ];

        // Act
        var result = CitizenReportsDataTable
            .FromForm(form)
            .WithData()
            .For(citizenReport1)
            .For(citizenReport2)
    ...
    [Fact]
    public void CitizenReportsDataTableGenerator_Should_Generates_Correct_DataTable_WhenNumberAnswers()
    {
        // Arrange
        var form = Fake.Form(DefaultLanguageCode, _numberQuestion);
        // No notes, no attachments
        var submission1 = Fake.CitizenReport(form.Id, NumberAnswer.Create(_numberQuestionId, 42), [], []);
        // No notes, 2 attachments
        var submission2 = Fake.CitizenReport(form.Id, NumberAnswer.Create(_numberQuestionId, 43), [],
            FakeAttachmentsFor(_numberQuestionId));
        // 2 notes, no attachments
        var submission3 = Fake.CitizenReport(form.Id, NumberAnswer.Create(_numberQuestionId, 44),
            FakeNotesFor(_numberQuestionId), []);
        // 2 notes, 2 attachments
        var submission4 = Fake.CitizenReport(form.Id, NumberAnswer.Create(_numberQuestionId, 45),
    ...
    [Fact]
    public void CitizenReportsDataTableGenerator_Should_Generates_Correct_DataTable_WhenRatingAnswer()
    {
        // Arrange
        var form = Fake.Form(DefaultLanguageCode, _ratingQuestion);
        // No notes, no attachments
        var submission1 = Fake.CitizenReport(form.Id, RatingAnswer.Create(_ratingQuestionId, 4), [], []);
        // No notes, 2 attachments
        var submission2 = Fake.CitizenReport(form.Id, RatingAnswer.Create(_ratingQuestionId, 5), [],
            FakeAttachmentsFor(_ratingQuestionId));
        // 2 notes, no attachments
        var submission3 = Fake.CitizenReport(form.Id, RatingAnswer.Create(_ratingQuestionId, 9),
            FakeNotesFor(_ratingQuestionId), []);
        // 2 notes, 2 attachments
        var submission4 = Fake.CitizenReport(form.Id, RatingAnswer.Create(_ratingQuestionId, 10),
    ...
    [Fact]
    public void CitizenReportsDataTableGenerator_Should_Generates_Correct_DataTable_WhenDateAnswer()
    {
        // Arrange
        ...
        // No notes, no attachments
        var submission1 = Fake.CitizenReport(form.Id, DateAnswer.Create(_dateQuestionId, date1), [], []);
        // No notes, 2 attachments
        var submission2 = Fake.CitizenReport(form.Id, DateAnswer.Create(_dateQuestionId, date2), [],
            FakeAttachmentsFor(_dateQuestionId));
        // 2 notes, no attachments
        var submission3 = Fake.CitizenReport(form.Id, DateAnswer.Create(_dateQuestionId, date3),
            FakeNotesFor(_dateQuestionId), []);
        // 2 notes, 2 attachments
        var submission4 = Fake.CitizenReport(form.Id, DateAnswer.Create(_dateQuestionId, date4),
    ...
    [Fact]
    public void CitizenReportsDataTableGenerator_Should_Generates_Correct_DataTable_WhenSingleSelectAnswer()
    {
        // Arrange
        var form = Fake.Form(DefaultLanguageCode, _singleSelectQuestion);
        // No notes, no attachments
        var submission1 = Fake.CitizenReport(form.Id,
            SingleSelectAnswer.Create(_singleSelectQuestionId, SelectedOption.Create(_option1Id, null)), [], []);
        // No notes, 2 attachments
        var submission2 = Fake.CitizenReport(form.Id,
        ...
        // 2 notes, no attachments
        var submission3 = Fake.CitizenReport(form.Id,
        ...
        // 2 notes, 2 attachments
        var submission4 = Fake.CitizenReport(form.Id,
    ...
    [Fact]
    public void CitizenReportsDataTableGenerator_Should_Generates_Correct_DataTable_WhenMultiSelectAnswer()
    {
        // Arrange
    ...
    [Fact]
    public void CitizenReportsDataTableGenerator_Should_Generates_Correct_DataTable_WhenMultipleQuestions()
    {
        // Arrange
        var form = Fake.Form(DefaultLanguageCode, _textQuestion,
            _numberQuestion,
        ...
        var submission1 = Fake.PartialCitizenReport(form.Id,
            ...
            numberAnswer: (NumberAnswer.Create(_numberQuestionId, 42), FakeNotesFor(_numberQuestionId),
                FakeAttachmentsFor(_numberQuestionId)),
            ratingAnswer: (RatingAnswer.Create(_ratingQuestionId, 3), FakeNotesFor(_ratingQuestionId),
                FakeAttachmentsFor(_ratingQuestionId)),
            singleSelectAnswer: (
                SingleSelectAnswer.Create(_singleSelectQuestionId,
                    SelectedOption.Create(_option4Id, "user written text")), FakeNotesFor(_singleSelectQuestionId),
                FakeAttachmentsFor(_singleSelectQuestionId)),
            multiSelectAnswer: (MultiSelectAnswer.Create(_multiSelectQuestionId, selection),
                FakeNotesFor(_multiSelectQuestionId), FakeAttachmentsFor(_multiSelectQuestionId))
        );

        var submission2 = Fake.PartialCitizenReport(form.Id,
        ...
        var submission3 = Fake.PartialCitizenReport(form.Id,
            textAnswer: (TextAnswer.Create(_textQuestionId, "some answer"), FakeNotesFor(_textQuestionId),
                FakeAttachmentsFor(_textQuestionId)),
            dateAnswer: (DateAnswer.Create(_dateQuestionId, _utcNow), FakeNotesFor(_dateQuestionId),
                FakeAttachmentsFor(_dateQuestionId)),
    ...
    [Fact]
    public void
        CitizenReportsDataTableGenerator_Should_Generates_Correct_DataTable_WhenMultipleQuestions_AndEmptyResponses()
    {
        // Arrange
        var form = Fake.Form(DefaultLanguageCode, _textQuestion,
            _numberQuestion,
        ...
        var submission = Fake.CitizenReport(form.Id);
    ...
    private object[] GetDefaultExpectedColumns(CitizenReportModel citizenReport)
    {
        return
        [
    ...
    private SubmissionNoteModel[] FakeNotesFor(Guid questionId)
    {
        return
        [
            new SubmissionNoteModel { QuestionId = questionId, Text = Note1 },
            new SubmissionNoteModel { QuestionId = questionId, Text = Note2 },
        ];
    }
