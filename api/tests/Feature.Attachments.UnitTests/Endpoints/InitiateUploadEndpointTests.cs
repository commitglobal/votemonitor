...
public class InitiateUploadEndpointTests
{
    ...
    public async Task ShouldInitiateFileUpload_AndReturnPresignedUrl()
    {
        ...
        var uploadPath =
            $"elections/{request.ElectionRoundId}/polling-stations/{request.PollingStationId}/form/{request.FormId}/attachments";
        ...
        await _repository
            .Received(1)
            .AddAsync(Arg.Is<AttachmentAggregate>(x => x.Id == request.Id));
