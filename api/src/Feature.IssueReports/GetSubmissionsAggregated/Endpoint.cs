    ...
    : Endpoint<Request, Results<Ok<Response>, NotFound>>
{
    ...
    {
        Get("/api/election-rounds/{electionRoundId}/issue-reports/forms/{formId}:aggregated-submissions");
        DontAutoTag();
        Options(x => x.WithTags("issue-reports"));
        Policies(PolicyNames.NgoAdminsOnly);
        Summary(s =>
        {
            s.Summary = "Gets aggregated issue report form submissions with all the notes and attachments";
        });
    }
        ...
        CancellationToken ct)
    {
        ...
        {
            formSubmissionsAggregate.AggregateAnswers(issueReport);
        }

        var tasks = issueReports.SelectMany(x => x.Attachments).Select(AttachmentModel.FromEntity)
        ...
        return TypedResults.Ok(new Response
        {
            ...
            Notes = issueReports.SelectMany(x => x.Notes).Select(NoteModel.FromEntity).ToArray(),
