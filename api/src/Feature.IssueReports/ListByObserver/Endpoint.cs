...
public class Endpoint(IAuthorizationService authorizationService, INpgsqlConnectionFactory dbConnectionFactory)
    : Endpoint<Request, Results<Ok<PagedResponse<ObserverIssueReportsOverview>>, NotFound>>
{
    public override void Configure()
    {
        Get("/api/election-rounds/{electionRoundId}/issue-reports:byObserver");
        DontAutoTag();
        Options(x => x.WithTags("issue-reports"));
        Policies(PolicyNames.NgoAdminsOnly);

        Summary(x => { x.Summary = "Issue reports aggregated by observer"; });
    }

    public override async Task<Results<Ok<PagedResponse<ObserverIssueReportsOverview>>, NotFound>> ExecuteAsync(
        Request req,
        CancellationToken ct)
    {
        ...
        var sql = """
                  ...
                  SELECT "MonitoringObserverId",
                         ...
                         "NumberOfIssuesSubmitted",
                         "FollowUpStatus"
                  FROM (SELECT MO."Id"                                            "MonitoringObserverId",
                               ...
                               (SELECT COUNT(1)
                                FROM "IssueReports" IR
                                WHERE IR."MonitoringObserverId" = MO."Id"
                                  AND IR."ElectionRoundId" = @electionRoundId) AS "NumberOfIssuesSubmitted",
                               (
                                   CASE
                  ...
                  ORDER BY
                      ...
                      CASE WHEN @sortExpression = 'NumberOfIssuesSubmitted ASC' THEN "NumberOfIssuesSubmitted" END ASC,
                      CASE WHEN @sortExpression = 'NumberOfIssuesSubmitted DESC' THEN "NumberOfIssuesSubmitted" END DESC
        ...
        int totalRowCount = 0;
        List<ObserverIssueReportsOverview> entries = [];

        using (var dbConnection = await dbConnectionFactory.GetOpenConnectionAsync(ct))
        {
            using var multi = await dbConnection.QueryMultipleAsync(sql, queryArgs);
            totalRowCount = multi.Read<int>().Single();
            entries = multi.Read<ObserverIssueReportsOverview>().ToList();
        }

        return TypedResults.Ok(
            new PagedResponse<ObserverIssueReportsOverview>(entries, totalRowCount, req.PageNumber, req.PageSize));
    }
    ...
    private static string GetSortExpression(string? sortColumnName, bool isAscendingSorting)
    {
        ...
        {
            return $"{nameof(ObserverIssueReportsOverview.ObserverName)} ASC";
        }

        var sortOrder = isAscendingSorting ? "ASC" : "DESC";

        if (string.Equals(sortColumnName, nameof(ObserverIssueReportsOverview.ObserverName),
                StringComparison.InvariantCultureIgnoreCase))
        {
            return $"{nameof(ObserverIssueReportsOverview.ObserverName)} {sortOrder}";
        }

        if (string.Equals(sortColumnName, nameof(ObserverIssueReportsOverview.Email),
                StringComparison.InvariantCultureIgnoreCase))
        {
            return $"{nameof(ObserverIssueReportsOverview.Email)} {sortOrder}";
        }

        if (string.Equals(sortColumnName, nameof(ObserverIssueReportsOverview.PhoneNumber),
                StringComparison.InvariantCultureIgnoreCase))
        {
            return $"{nameof(ObserverIssueReportsOverview.PhoneNumber)} {sortOrder}";
        }

        if (string.Equals(sortColumnName, nameof(ObserverIssueReportsOverview.Tags),
                StringComparison.InvariantCultureIgnoreCase))
        {
            return $"{nameof(ObserverIssueReportsOverview.Tags)} {sortOrder}";
        }

        if (string.Equals(sortColumnName, nameof(ObserverIssueReportsOverview.NumberOfFlaggedAnswers),
                StringComparison.InvariantCultureIgnoreCase))
        {
            return $"{nameof(ObserverIssueReportsOverview.NumberOfFlaggedAnswers)} {sortOrder}";
        }

        if (string.Equals(sortColumnName, nameof(ObserverIssueReportsOverview.NumberOfIssuesSubmitted),
                StringComparison.InvariantCultureIgnoreCase))
        {
            return $"{nameof(ObserverIssueReportsOverview.NumberOfIssuesSubmitted)} {sortOrder}";
        }
