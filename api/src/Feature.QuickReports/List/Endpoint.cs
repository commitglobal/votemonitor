...
public class Endpoint(
    ...
    : Endpoint<Request, Results<Ok<List<QuickReportModel>>, NotFound, BadRequest<ProblemDetails>>>
{
    ...
    public override async Task<Results<Ok<List<QuickReportModel>>, NotFound, BadRequest<ProblemDetails>>> ExecuteAsync(Request req, CancellationToken ct)
    {
        ...
        var quickReports = await quickReportRepository.ListAsync(new ListQuickReportsForUserSpecification(req.ElectionRoundId, req.ObserverId), ct);
        var quickReportIds = quickReports.Select(x => x.Id).ToArray();

        var quickReportAttachments = await quickReportAttachmentRepository.ListAsync(new ListQuickReportAttachmentsForUserSpecification(req.ElectionRoundId, req.ObserverId, quickReportIds), ct);
