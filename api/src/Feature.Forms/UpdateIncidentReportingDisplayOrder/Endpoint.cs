...
namespace Feature.Forms.UpdateIncidentReportingDisplayOrder;
    ...
    IRepository<FormAggregate> formsRepository) : Endpoint<Request, Results<NoContent, NotFound, ProblemDetails>>
{
    ...
    {
        Put("/api/election-rounds/{electionRoundId}/incident-reporting-forms:display-order");
        Description(x => x.Accepts<Request>());
        DontAutoTag();
        Options(x => x.WithTags("incident-reporting-forms", "forms"));
        Policies(PolicyNames.NgoAdminsOnly);
        Summary(s =>
        {
            s.Summary =
                "Updates display order for published incident reporting forms belonging to the current monitoring NGO";
        });
    }
        ...
        CancellationToken ct)
    {
        ...
        var forms = await formsRepository.ListAsync(
            new GetIncidentReportingFormsByIdsSpecification(req.ElectionRoundId, req.NgoId, formIds), ct);
