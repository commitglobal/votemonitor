...
public class ListEndpointTests : IClassFixture<HttpServerFixture>
{
    ...
    public async Task Should_UseDefaultPagination_WhenNoFilterAndNoPagination()
    {
        ...
        // Act
        var (response, result) = await Fixture.PlatformAdmin.POSTAsync<List.Endpoint, List.Request, PagedResponse<PollingStationModel>>(new List.Request());
    ...
    public async Task Should_ListDataPaginated()
    {
        ...
        // Act
        var (response, result) = await Fixture.PlatformAdmin.POSTAsync<List.Endpoint, List.Request, PagedResponse<PollingStationModel>>(request);
