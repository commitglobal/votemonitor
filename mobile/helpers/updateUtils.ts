...
// Access 'extra' properties from update manifests (useUpdate)

const isAvailableUpdateCritical = (updatesSystem: any) => {
  const criticalIndexCurrent =
    updatesSystem?.currentlyRunning?.manifest?.extra?.expoClient?.extra?.updateCritical ?? 0;
  const criticalIndexUpdate =
    updatesSystem?.availableUpdate?.manifest?.extra?.expoClient?.extra?.updateCritical ?? 0;
