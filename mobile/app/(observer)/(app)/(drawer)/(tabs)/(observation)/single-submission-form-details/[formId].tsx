...
const SingleFormDetails = () => {
  const { t } = useTranslation(["form_overview", "common"]);
  const { formId, language, submissionId } = useLocalSearchParams<SearchParamsType>();
  const { isOnline } = useNetInfoContext();

  if (!formId || !language || !submissionId) {
    return <Typography>SingleFormDetails Incorrect page params</Typography>;
  }
  ...
  const onConfirmFormLanguage = (formId: string, language: string) => {
    ...
    router.replace(
      `/single-submission-form-details/${formId}?language=${language}&submissionId=${submissionId}`,
    );
