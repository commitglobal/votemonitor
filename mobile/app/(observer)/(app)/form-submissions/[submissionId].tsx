...
const FormSubmissions = () => {
  ...
  const goToNextQuestion = (nextQuestion: ApiFormQuestion | null) => {
    if (nextQuestion) {
      router.replace(
        `/form-questionnaire/${nextQuestion.id}?formId=${formId}&language=${language}&submissionId=${submissionId}`,
      );
    } else {
      return router.back();
    }
  ...
  const goToPrevQuestion = () => {
    ...
    if (prevQ) {
      router.replace(
        `/form-questionnaire/${prevQ.id}?formId=${formId}&language=${language}&submissionId=${submissionId}`,
      );
    }
  };
    ...
    try {
      ...
      if (activeElectionRound?.id) {
        ...
        queryClient.invalidateQueries({
          queryKey: AttachmentsKeys.attachments(activeElectionRound?.id, submissionId),
        });
