...
export const statisticsCacheKey = {
  all: (electionRoundId: string, dataSource: DataSources) =>
    ['election-round-statistics', electionRoundId, dataSource] as const,
};
...
export function useElectionRoundStatistics(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await API.get<MonitoringNgoStats>(
