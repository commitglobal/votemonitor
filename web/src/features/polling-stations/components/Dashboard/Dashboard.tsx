...
function usePollingStations({pageNumber, pageSize}: PageParameters): UseQueryResult<PageResponse<PollingStation>, Error> {
  return useQuery({
    queryKey: ['pollingStations', pageNumber, pageSize],
    queryFn: async () => {
      const response = await authApi.post<PageResponse<PollingStation>>('/polling-stations:list', { pageNumber, pageSize });
...
export default function PollingStationsDashboard(): ReactElement {
  return (
    <>
      ...
      <div className='mt-6'>
        <DataTable columns={pollingStationColDefs} pagedQuery={usePollingStations} />
      </div>
    </>
  );
}
