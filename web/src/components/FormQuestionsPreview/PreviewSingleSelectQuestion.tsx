...
function PreviewSingleSelectQuestion({ code, questionId, text, helptext, options }: PreviewSingleSelectQuestionProps) {
  ...
  return (
    <div className='grid gap-2'>
      ...
      <RadioGroup
        onValueChange={(value) => {
          const newAnswer: SingleSelectAnswer = {
            $answerType: AnswerType.SingleSelectAnswerType,
            questionId,
            selection: { optionId: value, text: '' },
          };
          setAnswer(newAnswer);
        ...
        {regularOptions?.map((option) => (
          <div className='flex items-center space-x-2' key={option.optionId}>
            ...
            <Label htmlFor={option.optionId} className='break-all'>
              {option.text}
            </Label>
          </div>
        ))}
        {!!freeTextOption && (
          <div className='flex items-center space-x-2' key={freeTextOption.optionId}>
            ...
            <Label htmlFor={freeTextOption.optionId} className='break-all'>
              {freeTextOption.text}
            </Label>
          </div>
        )}
      ...
      {!!localAnswer?.selection?.optionId && localAnswer?.selection?.optionId === freeTextOption?.optionId && (
        <Textarea
          onChange={(e) => {
            const newAnswer: SingleSelectAnswer = {
              ...localAnswer!,
              selection: { ...localAnswer!.selection, text: e.target.value },
            };
            setAnswer(newAnswer);
            setLocalAnswer(newAnswer);
          }}
