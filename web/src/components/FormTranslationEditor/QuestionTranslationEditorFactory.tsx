...
import { questionsIconMapping } from '../questionsEditor/utils';
import SelectQuestionTranslationEditor from './SelectQuestionTranslationEditor';
import RatingQuestionTranslationEditor from './RatingQuestionTranslationEditor';
import NumberQuestionTranslationEditor from './NumberQuestionTranslationEditor';
import TextQuestionTranslationEditor from './TextQuestionTranslationEditor';

interface QuestionTranslationEditorFactoryProps {
  questionIndex: number;
  activeQuestionId: string | undefined;
  setActiveQuestionId: (questionId: string | undefined) => void;
}

export default function QuestionTranslationEditorFactory({
  questionIndex,
  activeQuestionId,
  setActiveQuestionId,
}: QuestionTranslationEditorFactoryProps): FunctionComponent {
      ...
      <Collapsible
        ...
        <CollapsibleContent className='px-4 pb-4 space-y-4'>
          ...
          {question.$questionType === QuestionType.TextQuestionType && (
            <TextQuestionTranslationEditor questionIndex={questionIndex} />
          )}

          {question.$questionType === QuestionType.NumberQuestionType && (
            <NumberQuestionTranslationEditor questionIndex={questionIndex} />
          )}

          {question.$questionType === QuestionType.RatingQuestionType && (
            <RatingQuestionTranslationEditor questionIndex={questionIndex} />
          )}

          {(question.$questionType === QuestionType.MultiSelectQuestionType ||
            question.$questionType === QuestionType.SingleSelectQuestionType) && (
            <SelectQuestionTranslationEditor questionIndex={questionIndex} />
          )}
