...
API.interceptors.response.use(
  ...
  async (error: any) => {
    ...
    const isTokenExpiredError = error.response?.headers['token-expired'] === 'true' || error.response?.status === 401;

    if (error.response && isTokenExpiredError && !originalRequest?.url?.includes('auth') && !originalRequest._retry) {
